...
void TIM4_IRQHandler(void){
	/*Vamos a evaluar si la interrupcion se dio por el timer (UIF) o por la captura del canal (CCxIF)*/
	...
	/*Evaluamos si la interrupcion la llamo el CCxIF*/
	else if(TIM4->SR & TIM_SR_CC4IF){
		/*guardamos el valor del Contador transferido por el ultimo valor de captura*/
		if((TIM4->CCER & TIM_CCER_CC1E)){
			/*guardamos el valor de la captura del canal 1*/
			auxTimeStamp = TIM4->CCR1;
		}
		else if((TIM4->CCER & TIM_CCER_CC2E)){
			/*guardamos el valor de la captura del canal 2*/
			auxTimeStamp = TIM4->CCR2;
		}
		else if((TIM4->CCER & TIM_CCER_CC3E)){
			/*guardamos el valor de la captura del canal 3*/
			auxTimeStamp = TIM4->CCR3;
		}
		else if((TIM4->CCER & TIM_CCER_CC4E)){
			/*guardamos el valor de la captura del canal 4*/
			auxTimeStamp = TIM4->CCR4;
		}
		else{
			__NOP();
		}
		/*Llamamos al callback de la captura*/
