...
//Con la funcion FasrConfig les asignamos las propiedades GPIO
	...
	GPIO_PinFastConfig(&handlerPB8, PIN_8, GPIOB);   	GPIO_Config(&handlerPB8);
	GPIO_PinFastConfig(&handlerPB9, PIN_9, GPIOB); 		GPIO_Config(&handlerPB9);
	GPIO_PinFastConfig(&handlerPA7, PIN_7, GPIOA);  	GPIO_Config(&handlerPA7);
	GPIO_PinFastConfig(&handlerPA6, PIN_6, GPIOA);		GPIO_Config(&handlerPA6);
	GPIO_PinFastConfig(&handlerPC0, PIN_0, GPIOC);		GPIO_Config(&handlerPC0);
	GPIO_PinFastConfig(&handlerPC1, PIN_1, GPIOC);		GPIO_Config(&handlerPC1);
	GPIO_PinFastConfig(&handlerPB0, PIN_0, GPIOB);		GPIO_Config(&handlerPB0);
	GPIO_PinFastConfig(&handlerPC10, PIN_10, GPIOC);	GPIO_Config(&handlerPC10);
	GPIO_PinFastConfig(&handler2, PIN_4, GPIOA);    	GPIO_Config(&handler2);    //controla el transistor 1
	GPIO_PinFastConfig(&handler3, PIN_6, GPIOB);    	GPIO_Config(&handler3);    //controla el transistor 2
...
void numbers(uint8_t PinNumber) {
	...
	case 0: {
		GPIO_WritePin(&handlerPB8, SET);		GPIO_WritePin(&handlerPA7,  SET);		GPIO_WritePin(&handlerPB0, SET);
		GPIO_WritePin(&handlerPC0, SET);		GPIO_WritePin(&handlerPC1,  SET);		GPIO_WritePin(&handlerPB9, SET);
		break;
	}

	case 1: {
		GPIO_WritePin(&handlerPB9, SET);		GPIO_WritePin(&handlerPB0, SET);
		break;
	}

	case 2: {
		GPIO_WritePin(&handlerPB8,  SET);		GPIO_WritePin(&handlerPA6, SET);		GPIO_WritePin(&handlerPB9, SET);
		GPIO_WritePin(&handlerPC1,  SET);		GPIO_WritePin(&handlerPC0, SET);
		break;
	}

	case 3: {
		GPIO_WritePin(&handlerPB8,  SET);		GPIO_WritePin(&handlerPA6, SET);		GPIO_WritePin(&handlerPB9, SET);
		GPIO_WritePin(&handlerPB0,  SET);		GPIO_WritePin(&handlerPC0, SET);
		break;
	}

	case 4: {
		GPIO_WritePin(&handlerPA7,  SET);		GPIO_WritePin(&handlerPB9, SET);
		GPIO_WritePin(&handlerPB0,  SET);		GPIO_WritePin(&handlerPA6, SET);
		break;
	}

	case 5: {
		GPIO_WritePin(&handlerPB8,  SET);		GPIO_WritePin(&handlerPA7,  SET);		GPIO_WritePin(&handlerPA6, SET);
		GPIO_WritePin(&handlerPB0,  SET);		GPIO_WritePin(&handlerPC0,  SET);
		break;
	}

	case 6: {
		GPIO_WritePin(&handlerPB8,  SET);		GPIO_WritePin(&handlerPA6, SET);		GPIO_WritePin(&handlerPC1, SET);
		GPIO_WritePin(&handlerPB0,  SET);		GPIO_WritePin(&handlerPC0, SET);		GPIO_WritePin(&handlerPA7, SET);

		break;
	}

	case 7: {
		GPIO_WritePin(&handlerPB9, SET);		GPIO_WritePin(&handlerPB8, SET);		GPIO_WritePin(&handlerPB0, SET);
		break;
	}

	case 8: {
		GPIO_WritePin(&handlerPB8, SET);		GPIO_WritePin(&handlerPA7,  SET);		GPIO_WritePin(&handlerPA6, SET);
		GPIO_WritePin(&handlerPB0, SET);		GPIO_WritePin(&handlerPC0, SET);		GPIO_WritePin(&handlerPC1, SET);;
	...
	case 9: {
		GPIO_WritePin(&handlerPB8, SET);		GPIO_WritePin(&handlerPA7,  SET);		GPIO_WritePin(&handlerPA6, SET);
		GPIO_WritePin(&handlerPB0, SET);		GPIO_WritePin(&handlerPC0, SET);	 	GPIO_WritePin(&handlerPB9, SET);
